...
# 경기당 평균 턴오버 갯수가 높은 순으로 팀명 정렬
teams = myframe.sort_values(by='Turnovers', axis=0, ascending=True)['Team']

# 경기당 평균 턴오버 갯수가 높은 순 정렬
turnovers = myframe.sort_values(by='Turnovers', axis=0, ascending=True)['Turnovers']

# turnovers 저장할 리스트
percentdata = []
percentdata = turnovers

# 저장된 turnovers 데이터 값을 내림차순으로 저장
...
# 수평 막대그래프 표현
plt.barh(teams, turnovers, height=-0.7, align='center', color=mycolor,
...
# 그래프 제목
plt.title('NBA 20-21 시즌 팀별 경기당 평균 턴오버')

# 데이터축(x축) 라벨 설정
plt.xlabel('경기당 평균 턴오버 횟수')

...
teams = myframe.sort_values(by='Field Goal Persent', axis=0, ascending=True)['Team']

# 필드 득점 성공률 높은 순 정렬
fgp = myframe.sort_values(by='Field Goal Persent', axis=0, ascending=True)['Field Goal Persent']

# fgp 저장할 리스트
percentdata = []
percentdata = fgp

# 저장된 fgp 데이터 값을 내림차순으로 저장
...
# 수평 막대그래프 표현
plt.barh(teams, fgp, height=-0.7, align='center', color=mycolor,
...
# 그래프 제목
plt.title('NBA 20-21 시즌 팀별 2점 득점 성공률')

# 데이터축(x축) 라벨 설정
plt.xlabel('2점 득점 성공률')

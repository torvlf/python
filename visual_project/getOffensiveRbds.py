...
PNG = '.png'
CHART_NAME = 'Offensive_Rebounds_Chart'
...
# 경기당 평균 공격리바운드 갯수가 높은 순으로 팀명 정렬
teams = myframe.sort_values(by='Offensive Rebounds', axis=0, ascending=True)['Team']

# 경기당 평균 공격리바운드 갯수가 높은 순 정렬
offencerbds = myframe.sort_values(by='Offensive Rebounds', axis=0, ascending=True)['Offensive Rebounds']

# offencerbds 저장할 리스트
percentdata = []
percentdata = offencerbds

# 저장된 offencerbds 데이터 값을 내림차순으로 저장
...
# 수평 막대그래프 표현
plt.barh(teams, offencerbds, height=-0.7, align='center', color=mycolor,
...
# 그래프 제목
plt.title('NBA 20-21 시즌 팀별 경기당 평균 공격 리바운드')

# 데이터축(x축) 라벨 설정
plt.xlabel('경기당 평균 공격 리바운드 성공횟수')

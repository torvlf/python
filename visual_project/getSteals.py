...
# 경기당 평균 스틸 갯수가 높은 높은 순으로 팀명 정렬
teams = myframe.sort_values(by='Steals', axis=0, ascending=True)['Team']

# 경기당 평균 스틸 갯수가 높은 높은 순 정렬
steals = myframe.sort_values(by='Steals', axis=0, ascending=True)['Steals']

# steals 저장할 리스트
percentdata = []
percentdata = steals

# 저장된 steals 데이터 값을 내림차순으로 저장
...
# 수평 막대그래프 표현
plt.barh(teams, steals, height=-0.7, align='center', color=mycolor,
...
# 그래프 제목
plt.title('NBA 20-21 시즌 팀별 경기당 평균 스틸')

# 데이터축(x축) 라벨 설정
plt.xlabel('경기당 평균 스틸 성공횟수')
